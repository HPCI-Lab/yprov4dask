...
class Documenter:
  ...
  def register_function_call(
    self, task_id: Key, func_name: str, func_module: str,
    ...
    created and it is linked with its arguments through a `wasUsedBy` relation.
    ...
    for arg, value in func_args.items():
      ...
      self.document.used(activity=task_id, entity=name)

    for task in depts:
      self.document.wasInformedBy(informed=task_id, informant=task.key)
    
    return (task_id, parameters)

  def register_function_result(
    self, task_id: Key, dtype: any, nbytes: int
  ):
    """
    Registers in the provenance document a the completion of function call, i.e.
